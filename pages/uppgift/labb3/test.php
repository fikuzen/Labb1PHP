<?php
	...
	// Create the DB Settings instance.
	$dbSettings = new DBSettings("phpskolan_labb3_test", "localhost", "root", "");
		
	// Test av Database.php
	$databaseTest = Database::test($dbSettings);
	if(count($databaseTest) == 0) {
		$bodyLeft .= "
					<div class='span3'>
						<p class='alert alert-success'>Database test ok</p>
					</div>";
	} else {
		// För varje fel så skriver vi ut felet.
		foreach($databaseTest as $error) {
			$bodyLeft .= "
		...
		$bodyLeft .= "<p class='span4 alert alert-error'>Database test fungerar ej</p>";	
	}
	
	$validatorTest = Validator::test();
	if(count($databaseTest) == 0) {
		$bodyLeft .= "
					<div class='span3'>
						<p class='alert alert-success'>Validator test ok</p>
					</div>";
	} else {
		// För varje fel så skriver vi ut felet.
		foreach($databaseTest as $error) {
		...
		// Testet gick ej igenom
		$bodyLeft .= "<p class='span4 alert alert-error'>Validator test fungerar ej</p>";	
	}
	
	// Creates the database instance and connect.
	$testDatabase = new Database();
	$testDatabase->Connect($dbSettings);
	
	/**
	 * Test av Login MVC
	 */
	$testResults = LoginModel::test($testDatabase);
	 ...
	 * Test av User MVC
	 */
	$testResults = UserModel::test($testDatabase);
	if (count($testResults) == 0) {
		$bodyLeft .= "
					<div class='span3'>
						<p class='alert alert-success'>Usertest ok</p>
					</div>";
	} else {
		...
		// Testet gick ej igenom
		$bodyLeft .= "<p class='span4 alert alert-error'>Usertest fungerar ej</p>";
	}
